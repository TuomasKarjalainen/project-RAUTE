...
from Pretraining.ssh_connector import *
from Pretraining.chunker import connection
...
def create_and_fit(df_features, df_target, test_size, norm, inputs, hidden_sizes, outputs ,activ, EPOCHS, loss, optimizer, lr, metrics=['mae', distance_from_true, 'mape'], dropout=None, with_predictions=False, save_model=False, path='./model.h5', csv_path='./logs/'):
    """
    ...
    if with_predictions == True:
        for i in range(EPOCHS):
            history = model.fit(train_norm, y_train,
                                epochs=1,
                                validation_data=(test_norm,y_test))
    ...
    if save_model == True:
        ...
        return model, history, train, test, model_name, y_train, y_test, train_norm, test_norm
